...
```shell
conda 分为 anaconda 和 miniconda。anaconda是包含一些常用包的版本（这里的常用不代表你常用 微笑.jpg），miniconda则是精简版，需要啥装啥，所以推荐使用miniconda。
# 下载地址
...
# 创建虚拟环境
# 创建python版本为 3.7、名字为 spiders 的虚拟环境。
conda create -n spiders python=3.7 # spiders 文件可以在Anaconda安装目录envs文件下找到。

# 激活环境 spiders
source activate spiders  # Linux
conda activate spiders  # Linux
activate your_env_name(虚拟环境名称)  # Windows
...
# 查看虚拟环境 spiders 下的 package
conda list spiders

# 删除虚拟环境 spiders
conda remove -n spiders --all 

# 安装package到 spiders 环境中
conda install -n spiders [package] 

# 删除环境 spiders 中的某个包 
conda remove --name spiders package_name  
```
